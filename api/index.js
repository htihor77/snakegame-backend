...
// api/index.js
const { json } = require("micro"); // micro-like handler via vercel node
const astra = require("../astraApi");
const { v4: uuidv4 } = require("uuid");

/**
 * Simple express-like handling: parse path & method
 * Endpoints:
 *  - POST /auth/signup
 *  - POST /auth/login
 *  - POST /scores
 *  - GET  /scores/top?level=1
 */
...
module.exports = async function (req, res) {
  ...
  try {
    ...
    console.error("Astra init error:", e.message || e);
    return sendJSON(res, 500, { error: "DB init error: " + (e.message || e) });
  }
  ...
  if (method === "POST" && url.startsWith("/auth/signup")) {
    try {
      const body = await json(req);
      const { email, password, name } = body || {};

      if (!email || !password || !name) {
        return sendJSON(res, 400, { error: "Email, password, and name required" });
      }

      // minimal validation (you already had these functions)
      const emailValid = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email);
      if (!emailValid) return sendJSON(res, 400, { error: "Invalid email" });

      // check existing
      const existing = await astra.findUserByEmail(email);
      if (existing) return sendJSON(res, 409, { error: "User already exists" });

      const created_at = new Date().toISOString();
      const user = { email, password, name: name.trim(), created_at };
      await astra.insertUser(user);

      const token = Buffer.from(JSON.stringify({ email, name: name.trim() })).toString("base64");
      return sendJSON(res, 200, { success: true, token, email, name: name.trim() });
    } catch (err) {
      console.error("Signup Error:", err);
      return sendJSON(res, 500, { error: "Signup failed", details: err.message || err });
    }
  }
  ...
  if (method === "POST" && url.startsWith("/auth/login")) {
    try {
      ...
      const user = await astra.findUserByEmail(email);
      if (!user || user.password !== password) {
        return sendJSON(res, 401, { error: "Invalid email or password" });
      }

      const token = Buffer.from(JSON.stringify({ email, name: user.name || "Player" })).toString("base64");
      return sendJSON(res, 200, { success: true, token, email, name: user.name });
    } catch (err) {
      console.error("Login Error:", err);
      return sendJSON(res, 500, { error: "Login failed", details: err.message || err });
    }
  }

  if (method === "POST" && url.startsWith("/scores")) {
    try {
      const body = await json(req);
      const { token, score, level = 1, mode = "classic" } = body || {};

      if (!token || typeof score !== "number") {
        return sendJSON(res, 400, { error: "Missing token or valid score" });
      }
      ...
      // Retrieve previous bests for same level
      const existing = await astra.getScoresByLevel(level);
      const userScores = (existing || []).filter(r => r.email === user.email);
      const bestExisting = userScores.length ? Math.max(...userScores.map(r => r.score || 0)) : -Infinity;

      if (score <= bestExisting) {
        return sendJSON(res, 200, { success: true, message: "Score not better", updated: false, bestScore: bestExisting });
      }

      // Insert new score doc
      const id = uuidv4();
      const created_at = new Date().toISOString();
      const scoreDoc = { id, email: user.email, name: user.name, score, level, mode, created_at };
      await astra.insertScoreDoc(scoreDoc);

      return sendJSON(res, 200, { success: true, message: "Score submitted", id, updated: true });
    } catch (err) {
      console.error("Score Submit Error:", err);
      return sendJSON(res, 500, { error: "Score submit failed", details: err.message || err });
    }
  }

  if (method === "GET" && url.startsWith("/scores/top")) {
    try {
      // parse query param ?level=1
      const u = new URL(req.url, `http://${req.headers.host}`);
      const level = parseInt(u.searchParams.get("level") || "1");
      const rows = await astra.getScoresByLevel(level);

      // sort descending by score, created_at desc, take top 20
      const arr = (rows || [])
        .sort((a, b) => {
          if (b.score !== a.score) return b.score - a.score;
          return new Date(b.created_at) - new Date(a.created_at);
      ...
      return sendJSON(res, 200, { success: true, level, count: arr.length, data: arr });
    } catch (err) {
      console.error("Get Scores Error:", err);
      return sendJSON(res, 500, { error: "Cannot fetch scores", details: err.message || err });
    }
  }
